...
$$
omsætning\sim N(\alpha_{sektor_i} + \beta_{sektor_i}markedsværdi{sektor_i}, \sigma^2_{sektor_i}),1,...,35
$$
...
Vi laver altså en reduktion fra M1 til M2 som er den model hvor skæring er den samme, og undersøger om skæringen kan komme fra den samme foreling p væriden siger det kan godt lade sig gøre. 


Nu vil jeg undrsøge sammde for skræing

```{r}
M1_til_M2_beta <- anova(lm(omsætningen ~ sektor : markedsværdien),
                        lm(omsætningen ~ sektor * markedsværdien))
M1_til_M2_beta
```
...
```{r}
confint(lm(omsætningen ~ markedsværdien))
```

Umiddelbart ud fra konfidens interval så vil jeg ikke sige at den fælles hældnign vil være nul, men derimod mellem 0.32 og 1.01.


### Angiv et 95%-konfidensinterval for spredningen på omsætningen.

```{r}
lmSUM <- summary(lm(omsætningen ~ markedsværdien))
lmSUM$sigma
```
...
Ovenstående viser konfidensinteval for den fællges hældning for de tre regressionslinjer:
