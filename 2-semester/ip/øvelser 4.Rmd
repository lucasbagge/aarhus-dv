...
colours = ['A', 'B']
things = ['C', 'D', 'E']
[(x, (y,z)) for x in colours for y in things for z in things if x != y and y != z and z != x]
```

```{python}
L = (['A', 'B'], ['C', 'D', 'E'])
[(x, (y,z)) for x in L[0] for y in L[1] for z in L[1] if x != y and y != z and z != x]
```

- der mangler lidt for at det spiller.
- Afklaring om hvad den skal vise.
...
test = [(x, (y,z)) for x in L[0] for y in L[1] for z in L[1] if x != y and y != z and z != x]
```
