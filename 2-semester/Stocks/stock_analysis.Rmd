...
```{r}
SPY_tibble <- tq_get("SPY",from="2016-01-01",to="2017-01-01")
...
highchart(type = "stock") %>% 
  ...
  hc_add_series(full_data %>% 
                ...
                name = "SAR",
                type = "line") %>% 
  hc_add_series(full_data %>% timetk::tk_xts(date_var = "date") %$% ADX, yAxis = 1, name = "ADX", type = "line") %>% 
  hc_add_series(full_data %>% timetk::tk_xts(date_var = "date") %$% DIn, yAxis = 1, name = "DIn", type = "line") %>% 
  hc_add_series(full_data %>% timetk::tk_xts(date_var = "date") %$% DIp, yAxis = 1, name = "DIp", type = "line")
```
